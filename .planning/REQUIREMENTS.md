...
| AGENT-CFG-01 | Phase 25 — Agentic Multi-Tenant Architecture | Pending |
| AGENT-CFG-02 | Phase 25 — Agentic Multi-Tenant Architecture | Pending |
...
| CACHE-01 | Phase 25 — Agentic Multi-Tenant Architecture | Pending |
...
- [ ] **AGENT-CFG-01**: `tenant_configs` collection en MongoDB almacena por user_id: modules on/off, language, brand_name, voice_system_prompt; cambios reflejan en siguiente llamada sin redeploy
- [ ] **AGENT-CFG-02**: `agent_instances` collection almacena por user_id: model, temperature, tools_enabled, prompt_history (últimas 5 versiones); hot-reload vía Redis cache con TTL 5min e invalidación inmediata para toggles
...
- [ ] **CACHE-01**: Redis Upstash cachea `tenant_config:{user_id}` con TTL 5min; toggle `modules.voice` invalida cache inmediatamente; costo estimado <$20/mes en tier básico
